...
const ChangeDetectionInReact = () => {
  return (
    <div>
      ...
      <p>After the change detection process is started, React executes the <code>render()</code> function that returns a new version of Virtual DOM tree. React then compares the old and the new Virtual DOM to find difference between them. When the difference is found, React creates a patch that updates the corresponding HTML DOM nodes.</p>
    </div>
  );
};
