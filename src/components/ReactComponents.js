...
const ReactComponents = () => {
  return (
    <div>
      ...
      <p><code>setState()</code> can take two kinds of parameters: an object literal or a function (<a href="https://reactjs.org/docs/react-component.html#setstate">link</a>).</p>
      ...
      <p>According to React Docs it should be avoided to copy props into the state in the constructor method. Setting initial state from props is possible only when it is explicitly defined that the prop updates won't be important for the component (<a href="https://reactjs.org/docs/react-component.html#constructor">link</a>).</p>
      ...
      <p><code>render()</code> method should be a pure function (means it should not modify component state, it should return the same result each time it is invoked, it should not directly interact with the browser).</p>
    </div>
  );
};
